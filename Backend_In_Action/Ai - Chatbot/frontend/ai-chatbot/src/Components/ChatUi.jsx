import React from 'react';

// --- Mock Data ---
const ACTIVE_USER = {
  name: 'Alexander Jameson',
  status: 'Online',
  avatar: 'https://placehold.co/40x40/6f42c1/white?text=AJ',
  color: 'text-green-500' 
};

const MESSAGES = [
  { id: 1, sender: 'Other', content: "Hello! I'm here to help you with your property inquiry.", time: '10:09 am' },
  { id: 2, sender: 'Self', content: "I'm looking for a modern 3-bedroom apartment. Do you have any available listings?", time: '10:41 am' },
  { id: 3, sender: 'Other', content: "Yes, I have one that matches your criteria perfectly. I'll send over the details now.", time: '12:25 pm' },
  { id: 4, sender: 'Self', content: "Great, thanks! Looking forward to seeing them.", time: '12:57 pm' },
];

// --- Core Components ---

// Combined Icon logic directly into a reusable function for simplicity
const renderBoxIcon = (name, size = '20px', className = '') => (
  <i 
    key={name}
    className={`bx ${name} ${className}`} 
    style={{ fontSize: size }} 
    aria-hidden="true"
  ></i>
);

const ChatMessage = ({ message, user }) => {
  const isSelf = message.sender === 'Self';
  const bubbleClass = isSelf
    ? 'bg-black text-white rounded-t-xl rounded-bl-xl ml-auto'
    : 'bg-gray-100 text-gray-800 rounded-t-xl rounded-br-xl mr-auto';

  return (
    <div className={`flex w-full ${isSelf ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`flex items-end max-w-xs md:max-w-md ${isSelf ? 'flex-row-reverse' : 'flex-row'} space-x-2`}>
        {/* Avatar for the Other person */}
        {!isSelf && (
            <img src={user.avatar} alt={user.name} className="w-8 h-8 rounded-full object-cover mb-2" />
        )}

        {/* Message Content */}
        <div className="flex flex-col">
            <div className={`p-3 shadow-md text-base ${bubbleClass}`}>
              {message.content}
            </div>

            <span className={`block text-xs text-gray-400 mt-1 ${isSelf ? 'text-right mr-1' : 'text-left ml-1'}`}>
              {message.time}
            </span>
        </div>
      </div>
    </div>
  );
};

const ChatWindow = ({ user, messages }) => (
  <div className="h-full w-full flex flex-col min-h-0 bg-white">
    
    {/* Header (Consolidated) */}
    <div className="flex justify-between items-center p-4 border-b border-gray-100 bg-white/95 backdrop-blur-sm sticky top-0 z-10 rounded-t-[2.5rem] md:rounded-t-[3rem]">
      <div className="flex items-center">
        <img src={user.avatar} alt={user.name} className="w-10 h-10 rounded-full mr-3 object-cover shadow-sm" />
        <div>
          <h3 className="font-bold text-lg text-gray-800">{user.name}</h3>
          <p className={`text-sm font-medium ${user.color}`}>
              {user.status}
          </p>
        </div>
      </div>
      <div className="flex space-x-4 text-gray-500">
        {renderBoxIcon("bx-phone-call", "24px", "hover:text-black transition cursor-pointer p-1 rounded-full hover:bg-gray-100")}
        {renderBoxIcon("bx-video", "24px", "hover:text-black transition cursor-pointer p-1 rounded-full hover:bg-gray-100")}
        {renderBoxIcon("bx-info-circle", "24px", "hover:text-black transition cursor-pointer p-1 rounded-full hover:bg-gray-100")}
      </div>
    </div>

    {/* Messages Area */}
    <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
      {messages.map(msg => (
        <ChatMessage key={msg.id} message={msg} user={user} />
      ))}
    </div>

    {/* Input (Consolidated) */}
    <div className="flex items-center p-4 border-t border-gray-100 bg-white sticky bottom-0 z-10 rounded-b-[2.5rem] md:rounded-b-[3rem]">
      
      {/* Attachment and Emoji Icons */}
      <div className="flex items-center space-x-3 text-gray-500 mr-3">
        {renderBoxIcon("bx-paperclip", "26px", "hover:text-black transition cursor-pointer")}
        {renderBoxIcon("bx-smile", "26px", "hover:text-black transition cursor-pointer")}
      </div>

      {/* Input Field */}
      <div className="relative flex-1">
        <input
          type="text"
          placeholder="Type a message..."
          className="w-full py-3 pl-4 pr-12 bg-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-black/10 transition text-gray-800 text-sm"
        />
        {/* Send Button */}
        <div className="absolute right-1 top-1/2 transform -translate-y-1/2 p-2 bg-black rounded-full cursor-pointer shadow-lg hover:bg-gray-800 transition">
          {renderBoxIcon("bx-send", "20px", "text-white transform -rotate-45 -mt-0.5")}
        </div>
      </div>
    </div>
  </div>
);

const ChatUi = () => {
  return (
    // Load Boxicons CSS CDN
    <>
      <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet' />
      <style>{`
        /* Custom scrollbar for clean aesthetic */
        .custom-scrollbar::-webkit-scrollbar {
          width: 6px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background-color: rgba(0, 0, 0, 0.15);
          border-radius: 10px;
        }
        .custom-scrollbar::-webkit-scrollbar-track {
          background: transparent;
        }
      `}</style>
      <div className="font-sans min-h-screen p-8 flex items-center justify-center bg-gray-100" style={{
        backgroundImage: 'linear-gradient(135deg, #a8dadc 0%, #457b9d 100%)'
      }}>
        {/* Main UI Container: Focused, centered chat column */}
        <div className="flex flex-col h-[90vh] w-full max-w-lg shadow-2xl rounded-[3rem] overflow-hidden bg-white">
          <ChatWindow user={ACTIVE_USER} messages={MESSAGES} />
        </div>
      </div>
    </>
  );
};

export default ChatUi;
